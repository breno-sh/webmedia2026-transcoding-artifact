#!/usr/bin/env python3
"""
Phase 3.2 OPTIMIZED — Vertical: 1× c5.4xlarge with custom AMI.
"""
import os, time, sys
...
def run():
    context = f"{INSTANCE_TYPE}-Vertical-OPT"
    ec2 = boto3.client('ec2', region_name=AWS_CONFIG['region'])
    instance_id, ssh_client = None, None
    log(f"=== Phase 3.2 OPTIMIZED Vertical: 1x {INSTANCE_TYPE} ===")
    log(f"AMI: {AWS_CONFIG['ami_id']} (ffmpeg pre-installed)")
    if not os.path.exists(INPUT_VIDEO):
        log(f"ERROR: Video not found", context); return
    ...
    try:
        ...
        if exit_status != 0:
            log(f"ERROR: FFmpeg failed (exit {exit_status}): {stderr.read().decode()[-200:]}", context); return
        ...
        total_end = time.time(); total_time = total_end - total_start
        if ssh_client:
        ...
        log("=" * 60, context)
        log(f"RESULTS — {INSTANCE_TYPE} Vertical OPTIMIZED", context)
...
if __name__ == "__main__":
    ...
    with open("c5_4xlarge_vertical_opt.log", "w") as logfile:
