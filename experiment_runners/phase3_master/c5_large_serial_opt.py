#!/usr/bin/env python3
"""
Phase 3.2 OPTIMIZED — Serial Baseline: 1× c5.large with custom AMI.
"""
import os, time, sys
...
def run():
    context = f"{INSTANCE_TYPE}-Serial-OPT"
    ec2 = boto3.client('ec2', region_name=AWS_CONFIG['region'])
    instance_id, ssh_client = None, None
    log(f"=== Phase 3.2 OPTIMIZED Serial: 1x {INSTANCE_TYPE} ===")
    ...
    try:
        ...
        log("=" * 60, context)
        log(f"RESULTS — {INSTANCE_TYPE} Serial OPTIMIZED", context)
...
if __name__ == "__main__":
    ...
    with open("c5_large_serial_opt.log", "w") as logfile:
